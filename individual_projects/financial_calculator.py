...
def choose_function():
    ...
    while True:
        ...
        calc_choice = input("Select an option(1-5): ")
...
def savings_time(goal_money):
    ...
    total_time = float(goal_money)/float(contri_val)
...
def budget_allocator(month_income):
    ...
    for category in range(int(category_amount)):
        ...
        cat_num += 1
    #Go through and ask what percent (as a float) they spend on each category (use a loop. This part will most likely be the inner function)
    def ask_percent():
        money_category = []
        for category in categories:
            percent_category = float(input(f"What percent is your {category}?: "))
            dollar_category = (percent_category/100) * month_income
            money_category.append(dollar_category)
        return money_category
    all_categories = ask_percent()
    ...
    for category in range(len(all_categories)):
        print(f"{categories[category]} is ${all_categories[category]:.2f}.")
...
def sales_price(org_cost):
    ...
    discount = input("What's the discount on the item?(as a whole number): ")
    ...
    percentage = (100 - float(discount))/100
...
while True:
    ...
    if user_choice == "1":
        savings_time(get_money(user_choice))
    elif user_choice == "2":
        compound_interest(get_money(user_choice))
    elif user_choice == "3":
        budget_allocator(get_money(user_choice))
    elif user_choice == "4":
        sales_price(get_money(user_choice))
    elif user_choice == "5":
        tip_calc(get_money(user_choice))
    ...
    another_calc = input("Do you want to do another calculation?(y/n): ")
    if another_calc == "y":
        continue
    elif another_calc == "n":
        break
