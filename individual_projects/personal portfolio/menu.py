...
def window():
    ...
    welcome_txt.config(background="cyan")
    welcome_txt.grid(row=1, column=0, columnspan=2, pady=20, padx=10)

    project_one = tk.Button(root, text="Sierpinski\nTriangle\nGenerator", command=lambda: show_desc("project_one", info_box), font=("Georgia", 15), width=12, height=3)
    project_one.grid(row=2, column=0, padx=10, pady=15)
    project_two = tk.Button(root, text="Shape\nCalculator", command=lambda: show_desc("project_two", info_box), font=("Georgia", 15), width=12, height=2)
