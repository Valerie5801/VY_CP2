...
def save_csv():
    ...
    try:
        ...
        with open("individual_projects//updated personal library//new_personal_library.csv", mode= "r") as sample:
...
def view_library():
    while True:
        view_how = input("Do you want to show only simple details(title and author only) or all details?(simple/all): ")
...
def remove_item():
    while True:
        print("\nPlease type the full title.")
        remove_title = input('Title of book you want to remove: ').title()
        found_book = False
...
def search_item():
    ...
    while True:
        print("\nYou can search by: \n1. Title \n2. Author")
        search_by = input("What would you like to search by?: ").title()
        added_books = ["Placeholder"]

        if search_by == "1":
            find_title = input("What's the title?: ").title()
            ...
            find_author = input("What's the author's full name?: ").title()
